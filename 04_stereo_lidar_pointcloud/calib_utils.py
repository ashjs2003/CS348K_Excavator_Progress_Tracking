...
# Scripts are intended to be run from this folder. These helpers still look in
# workflow-local config folders for ad-hoc filenames, while final pipeline
# artifacts come from the repo-level config.yaml.
BASE_DIR = Path(__file__).resolve().parent
SEARCH_DIRS = [
    BASE_DIR,
    BASE_DIR.parent,
    BASE_DIR.parent / "01_rgb_calibration" / "config",
    BASE_DIR.parent / "02_stereo_calibration" / "config",
    BASE_DIR.parent / "03_lidar_camera_calibration" / "config",
]
...
def load_lidar_to_rgb1():
    ...
    path = calibration_file("lidar_to_rgb1_extrinsics")
    data = np.load(path)
...
def load_stereo_rgb1_to_rgb2():
    ...
    path = calibration_file("stereo_rgb1_rgb2_extrinsics")
    data = np.load(path)
    return (
        path,

...
Run:
    ...
    python 01_capture_checkerboard_images.py --camera L
...
from utils.project_config import open_configured_camera

from calib_targets import FRAME_SIZE, prompt_camera, resolve_camera

CHECKERBOARD = (9, 6)


def parse_args():
    parser = argparse.ArgumentParser(description="Capture checkerboard images for the L or R RGB camera.")
    parser.add_argument(
        "--camera",
        choices=["L", "R", "l", "r", "left", "right", "rgb1", "rgb2"],
        default=None,
        help="Which camera to capture: L or R. If omitted, you will be prompted.",
    )
...
def main():
    ...
    try:
        cap = open_configured_camera(camera_index, FRAME_SIZE)

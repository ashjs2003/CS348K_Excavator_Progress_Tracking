# Final Project Writeup

**Project:** Edge-Deployable Perception Stack for Earthwork Progress Tracking  
**Team:** Ashmitha Jaysi Sivakumar (ashjs@stanford.edu), Yashasvini Gopalan (ygopalan@stanford.edu)

## Sections

# Stage 1: CS348K Project, Understanding Camera Setup on toy Excavator
1. [Background and Setup](Background_and_Setup.md)
2. Approach
   - [Data](Data.md)
   - [Transforms](Transforms.md)
   - [DepthMap](DepthMap.md)
   - [VolumeEstimation](VolumeEstimation.md)
3. Evaluation
   - [Transform Evaluation](Evaluation_Transforms.md)
   - [Depth Map Evaluation](Evaluation_DepthMap.md)
   - [Volume Evaluation](Evaluation_VolumeEstimation.md)
4. [Conclusion](Conclusion.md)

# Stage 2: Real Camera Setup at Excavator Height
1. [SensorSetup](SensorSetup.md)
